...
const Form = () => {
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      const lowercaseEmail = outlookMail.toLowerCase();

      // Update the displayed email to lowercase
      setOutlookMail(lowercaseEmail);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (loading || otpLoading || verifyLoading) {
      return;
    }
    setLoading(true);
    ...
    formData.append("semester", semester);
    formData.append("subject", subject);
    // Convert email to lowercase when appending to formData
    formData.append(
      "outlookMail",
      `${outlookMail.toLowerCase()}@muj.manipal.edu`
    );
    formData.append("passedNptel", passedNptel);
    formData.append("otherExam", otherExam);
  ...
  const handleEmailChange = (e) => {
    setOutlookMail(e.target.value.toLowerCase());
  };
                ...
                <div className="space-y-2">
                  ...
                  >
                    <input
                      ...
                      className="w-full px-3 sm:px-4 py-2 pr-24 border border-amber-300 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 transition duration-150 ease-in-out text-sm sm:text-base bg-white bg-opacity-80"
                    />
                    <span className="absolute inset-y-0 right-1 top-1 bottom-1 flex items-center text-gray-800 text-sm sm:text-base bg-gray-200 px-2 md:px-8 rounded-md">
                      @muj.manipal.edu
                    </span>
                  </motion.div>
                  ...
                  <motion.button
                    ...
                    whileHover={{ scale: otpLoading || loading ? 1 : 1.05 }}
                    whileTap={{ scale: otpLoading || loading ? 1 : 0.95 }}
                    className={`w-full sm:w-auto py-2 px-4 rounded-md transition duration-150 ease-in-out font-medium text-sm sm:text-base ${
                      otpLoading || loading
                        ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                        : "bg-gradient-to-r from-amber-500 to-amber-600 text-white hover:from-amber-600 hover:to-amber-700"
                    }`}
                ...
                {otpSent && (
                  <div className="space-y-2">
                    ...
                    <motion.button
                      ...
                      whileHover={{
                        scale: verifyLoading || loading ? 1 : 1.05,
                      }}
                      whileTap={{ scale: verifyLoading || loading ? 1 : 0.95 }}
                      className={`w-full sm:w-auto py-2 px-4 rounded-md transition duration-150 ease-in-out font-medium text-sm sm:text-base ${
